...
| `POST` | `/api/v1/queue/optical-reg/measure` | At a pen-change pause, run a camera measurement of how far this pen-change left the next layer off the first (needs a camera-registration-calibrated install and a job created with `optical_reg: true`). Body: `{"probe_mm": 4.0}` optional — the nominal offset of the probe cross, defaults to the configured value; a larger value retries with the crosses further apart. The result lands in the WebSocket state as `optical_reg` (`status`, `dx_mm`, `dy_mm`, `confidence`, `probe_mm`, `reason`, plus `ready` — whether this run has a reference mark at all); apply it with `/api/v1/queue/nudge-origin`. A reading further off than `optical_reg_max_correction_mm` is reported as a failure rather than clamped to the limit. | Active job is not in `awaiting_pen_change`; camera disabled; camera not calibrated for optical registration; the run has no reference mark (the job was created without `optical_reg`, or it resumed past the pen change that draws it). |
